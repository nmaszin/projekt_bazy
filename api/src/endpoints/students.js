...
router.get('/students', controller(async (req, res) => {
    const students = (await Student.selectAll()).map(flattenSelect)
    res.send({ data: students })
}))


router.get('/students/:id(\\d+)', controller(async (req, res) => {
...
router.post('/students', validator(StudentForm), controller(async(req, res) => {
    await Student.insert(req.body)
    res.status(201).send({ message: 'Created' })
}))


router.put('/students/:id(\\d+)', validator(StudentForm), controller(async (req, res) => {
    const id = parseInt(req.params.id)
...
router.delete('/students/:id', controller(async (req, res) => {
    const id = parseInt(req.params.id)
